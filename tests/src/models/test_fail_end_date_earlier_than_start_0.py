...
def test_event_list_date_teime():
    calendar_data = [
        {
            "kind": "calendar#event",
            ...
            "start": {"dateTime": "2024-09-15 21:00:03"},
            "end": {"dateTime": "2024-09-15 20:00:03"},
    ...
    with pytest.raises(ValidationError, match=r"End\ time\ must\ be\ greater"):
        event_list(events=calendar_data)


def test_event_list_date():
    calendar_data = [
        {
            "kind": "calendar#event",
            "etag": '"3452471884512000"',
    ...
    with pytest.raises(ValidationError, match=r"End\ date\ must\ be\ greater"):
        event_list(events=calendar_data)
